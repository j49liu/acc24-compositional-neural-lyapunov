...
def compute_sos_V_inclusion(system, sos_V, c2_P, c2_V):
    ...
    symbolic_vars = [x1, x2]   
    V = sympy_to_dreal(sos_V, dict(zip(symbolic_vars, x)))
...
def self_sos_gain_verifier(system, sos_V, c2_V, total_gain):
    epsilon = -1e-4
    xlim = system.domain
    x = [Variable(f"x{i}") for i in range(1, len(system.symbolic_vars) + 1)]
...
def sos_gain_verifier(system1, sos_V1, c1, system2, sos_V2, c2):
    xlim = system1.domain
    ylim = system2.domain
    ...
    x1, x2 = sympy.symbols('x1 x2')
    symbolic_vars = [x1, x2]    
    V1 = sympy_to_dreal(sos_V1, dict(zip(symbolic_vars, x)))
    V2 = sympy_to_dreal(sos_V2, dict(zip(symbolic_vars, y)))
